...
class CharacterMotorMovement {
	...
	var doubleMaxSidewasSpeed : float = 30.0;
	var doubleMaxForwardSpeed : float = 30.0;
	var doubleMaxBackwardsSpeed : float = 30.0;
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	...
	if (desiredMovementDirection == Vector3.zero)
		...
		var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? (Input.GetKey(KeyCode.LeftShift)==true? movement.doubleMaxForwardSpeed :movement.maxForwardSpeed ): (Input.GetKey(KeyCode.LeftShift)==true? movement.doubleMaxBackwardsSpeed :movement.maxBackwardsSpeed)) / (Input.GetKey(KeyCode.LeftShift)==true ?movement.maxSidewaysSpeed: movement.maxSidewaysSpeed);
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude *(Input.GetKey(KeyCode.LeftShift) ?movement.maxSidewaysSpeed: movement.maxSidewaysSpeed);
		return length;
	}
}
