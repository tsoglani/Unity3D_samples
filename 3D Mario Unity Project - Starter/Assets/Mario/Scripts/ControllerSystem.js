...
private var canJumpOnAir            :boolean            =true;
...
//private var defaultSpeed            : float             =currentSpeed;
...
function Start 					() {												// initialize variables
	...
	characterController.tag = "Player";			//	speed								// set tag name to 'Player'
...
function UpdateMoveDirection 	() {												// motor, ani, and direction of player			
	...
	{										
		...
		Fall ();
		JumpOnAir();																	// check if player is falling from jump		
	}
	Killzone ();		
	// print(currentSpeed);															// check for player triggering killzone box
}
function Update 				() {									
																														// loop for controller
	if ( isControllable )															// if player controllable, then move character
	{
		...
		collisionFlags = characterController.Move ( movement );						// move the character controller ...player moving
  ...
  //  currentSpeed= 10;   
                                                                                           ...
                                                                                           // in case current speed has changed 												// example usage of show/hide functions	
}
...
function Sprint 				() {
 if ( canSprint){ 
  if ( moveSpeed > speedRun && moveSpeed <= speedSprint&&Input.GetKey(KeyCode.LeftShift) ){																					// sprints player
Message ( "Ani State: Sprinting" );		   
 animation.Play(aniSprint.name);
...
function Jump_2					() {												// jump 2 in combo
 if(canJumpAll) { 
 if(Input.GetKey(KeyCode.Space)&&!isJumping_1&&isJumping_2&&!isJumping_3&&(curTime+jumpComboTime)>Time.time){
                print("Jump 2 "); 
 	            	isJumping_2 = false;										// set jump 1 to false so it just does it once
				curTime = Time.time;												// grab the current actual time to use for testing against next button press
				animation.CrossFade ( aniJump_2.name );								// play jump 1 animation
				currentJumpHeight = jump_2;											// set jump 1 height to current jump height
				inAirVelocity.y = currentJumpHeight;								// set current jump to in Air Y (don't have to by-pass twice, just doing it)
				Message ( "Ani State: Jump 2" );	 
				yield ; 
				
 	             isJumping_3 = true;	
 }
 if(!isJumping_1&&isJumping_2&&!isJumping_3&&(curTime+jumpComboTime)<Time.time){
       isJumping_2 = false;
       isJumping_1 = true;
 ...
 if(canJumpAll) { 
		 if(Input.GetKey(KeyCode.Space)&&!isJumping_1&&!isJumping_2&&isJumping_3&&(curTime+jumpComboTime)>Time.time){
                print("Jump 3 "); 
 	           												...
 	           												// set jump 1 to false so it just does it once											// grab the current actual time to use for testing against next button press
				animation.CrossFade ( aniJump_3.name );								// play jump 1 animation
				currentJumpHeight = jump_3;											// set jump 1 height to current jump height
				inAirVelocity.y = currentJumpHeight;								// set current jump to in Air Y (don't have to by-pass twice, just doing it)
				Message ( "Ani State: Jump 3" );	 
				//yield; 
				   isJumping_3 = false;
				   isJumping_1 = true;
 }	
  ...
  isJumping_1 = true;	
 	...
 	else if(!isJumping_1&&!isJumping_2&&!isJumping_3&&(curTime+jumpComboTime)<Time.time){
 
  isJumping_3 = false;
  isJumping_2 = false;
  isJumping_1 = true;	
 }	
 																				// jump 3 in combo (final jump)
 }
}
...
inAirVelocity.y+=jump_onAir;
canJumpOnAir=false;
print("jumping on air");
}
}
